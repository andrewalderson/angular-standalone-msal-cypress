...
import { _MSAL_CONFIG } from './app/auth/msal.tokens';
 ...
 * fetch the confog here before bootstrapping and create an InjectionToken with the config
 * The bonus in this is that the redirect for the authentication will happen
 * before the app is bootstrapped
 */
fetch('./assets/msal.config.json')
  .then((response) => response.json())
  .then((msalConfig) =>
    bootstrapApplication(AppComponent, {
      providers: [
        ...appConfig.providers,
        { provide: _MSAL_CONFIG, useValue: msalConfig },
      ],
    })
